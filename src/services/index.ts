...
export const modelList = async (brand: String) => {
  ...
  let models: DataType[] = [];
  modelos.map((d: DataType, i: Number) => models.push(d));
  return modelos;
};
...
export const yearList = async (brand: String, model: String) => {
  ...
  let years: DataType[] = [];
  anos.map((d: DataType, i: Number) => years.push(d));
  return anos;
};

export const getDetails = async ({ brand, model, year }: ResultsType) => {
  let res = await fetch(`${BASE_URL}/carros/marcas/${brand}/modelos/${model}/anos/${year}`);
